...
class Notice_Board_View_M extends CI_Model {
  ...
  public function pagination_seach($type, $search_text) {
    // 검색 타입에 따른 컬럼 설정
    switch ($type) {
      case '제목만':
  ...
  public function GetBoardTotal() {
    $result = $this->db->query("SELECT idx FROM boards WHERE board_type = 'notice' ;")->num_rows();
    return $result;
  }
