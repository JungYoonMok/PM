...
class Free_Board_View_C extends CI_Controller
{
  ...
  public function search() {
    ...
    $data['list'] = $this->FBM->search($config['per_page'], $page, $type, $search_text);
    if($data['list']) {
      echo json_encode([ 'state' => TRUE, 'message' => '검색 성공', 'data' => $data ]);
    } else {
      echo json_encode([ 'state' => FALSE, 'message' => '검색 실패', 'data' => $data ]);
    }
  }
