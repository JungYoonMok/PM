    ...
    <form id="registerForm" action="/register_C/register" method="post"
      ...
      <div class="flex gap-2 w-full">
        <div class="w-full flex flex-col gap-2">
          ...
          <input name='UserID' placeholder="abc123"
      ...
      <div class="flex gap-2">
        <div class="w-full flex flex-col gap-2">
          <h2>비밀번호</h2>
          <input name="user_password_1" placeholder="비밀번호 입력"
            class="w-full font-black duration-200 border border-gray-500 hover:bg-[#4f4f4f] focus:bg-[#2f2f2f] bg-[#3f3f3f] p-3 h-[50px] rounded outline-none"
            type="password" />
        </div>
        <div class="w-full flex flex-col gap-2">
          <h2>비밀번호 확인</h2>
          <input name="user_password_2" placeholder="비밀번호 입력"
            class="w-full font-black duration-200 border border-gray-500 hover:bg-[#4f4f4f] focus:bg-[#2f2f2f] bg-[#3f3f3f] p-3 h-[50px] rounded outline-none"
            type="password" />
        </div>
      </div>
...
<script>
  ...
  $(document).ready(function () {
    $('#registerButton').click(function () {
      ...
      var password_1 = $('#user_password_1').val();
      var password_2 = $('#user_password_2').val();
      $.ajax({
        url: '/Register_C/register',
        type: 'post',
        data: { username, password_1, password_2 },
        dataType: 'json',
